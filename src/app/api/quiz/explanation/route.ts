...
export async function POST(req: NextRequest) {
  try {
    ...
    const uidRl = await enforceRateLimit(`ai:explanation:${auth.uid}`, Limits.AI_EXPLANATION_PER_USER);
    ...
    if (result.error) {
      ...
      if (result.error === 'EXPLANATION_TIMEOUT' || result.error.includes('EXPLANATION_TIMEOUT') || result.error.includes('TIMEOUT')) return NextResponse.json({ error: 'Explanation generation timed out. Please try again.' }, { status: 504 });
