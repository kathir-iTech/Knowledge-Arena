...
import { COLLECTIONS, QUIZ_FINISHED, QUIZ_ARCHIVED } from '@/lib/constants';
  ...
  try {
    ...
    await db.runTransaction(async (tx) => {
      ...
      if (!isCreator(quiz, auth.uid)) {
        throw new Error('Only the Commander can archive this arena');
      }
      if (quiz.status !== QUIZ_FINISHED) {
        throw new Error(`Cannot archive a battle in state: ${quiz.status}`);
      }
      tx.update(quizRef, { status: QUIZ_ARCHIVED, archived: true });
    });

    await writeBattleLog({ quizId, event: 'battle_archived', actor: auth.uid, actorRole: 'commander' });
    return NextResponse.json({ ok: true });
  } catch (err: any) {
    console.error('[Battle/archive]', err?.message);
