        ...
        <CardContent className="space-y-12 py-10">
          ...
          <div className="flex justify-center items-end gap-2 md:gap-8 h-80 mt-10">
            ...
            {ranked[1] && (
              <div className="flex flex-col items-center gap-3 bg-secondary/30 p-6 rounded-t-3xl w-32 md:w-44 h-56 border-t-4 border-slate-400/50 shadow-lg relative group transition-all hover:bg-secondary/40">
                ...
                <Avatar className="h-16 w-16 md:h-24 md:w-24 border-4 border-slate-400 shadow-xl group-hover:scale-110 transition-transform"><AvatarFallback className="text-3xl">{ranked[1].avatar}</AvatarFallback></Avatar>
                <div className="text-center">
                  <p className="font-bold text-sm truncate w-24 md:w-36">{ranked[1].name}</p>
            ...
            {ranked[0] && (
              <div className="flex flex-col items-center gap-3 bg-primary/10 p-6 rounded-t-3xl w-40 md:w-60 h-72 border-t-8 border-yellow-500 shadow-2xl shadow-yellow-500/10 relative group transition-all hover:bg-primary/20">
                ...
                <Avatar className="h-24 w-24 md:h-32 md:w-32 border-4 border-yellow-500 shadow-2xl shadow-yellow-500/20 group-hover:scale-110 transition-transform"><AvatarFallback className="text-5xl">{ranked[0].avatar}</AvatarFallback></Avatar>
                <div className="text-center mt-2">
                  <p className="font-black text-xl truncate w-32 md:w-52 uppercase tracking-wide">{ranked[0].name}</p>
            ...
            {ranked[2] && (
              <div className="flex flex-col items-center gap-3 bg-secondary/20 p-6 rounded-t-3xl w-32 md:w-40 h-44 border-t-4 border-amber-700/50 shadow-lg relative group transition-all hover:bg-secondary/30">
                ...
                <Avatar className="h-14 w-14 md:h-20 md:w-20 border-4 border-amber-700 shadow-xl group-hover:scale-110 transition-transform"><AvatarFallback className="text-2xl">{ranked[2].avatar}</AvatarFallback></Avatar>
                <div className="text-center">
          ...
          <div className="space-y-3 max-h-72 overflow-y-auto pr-2 custom-scrollbar border-t border-border/50 pt-8">
            ...
            {ranked.slice(3).map((p, i) => (
              <div key={p.id} className="flex justify-between items-center p-5 bg-secondary/20 rounded-2xl border border-border/30 hover:bg-secondary/30 transition-colors group">
                <div className="flex items-center gap-6">
                  <span className="font-mono text-xl text-muted-foreground w-8 group-hover:text-primary transition-colors">{i + 4}</span>
                  <Avatar className="h-12 w-12 border-2 border-border group-hover:border-primary/50 transition-colors"><AvatarFallback className="bg-background">{p.avatar}</AvatarFallback></Avatar>
